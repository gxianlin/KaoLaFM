...
/**
 * 描述：自定义可下来加载和上拉刷新的RecyclerView
 * 作者：gong.xl
 * 创建日期：2017/11/28 上午9:57
 * 修改日期: 2017/11/28
 * 修改备注：
...
public class CustomRecyclerView extends RecyclerView implements View.OnTouchListener {
     ...
     * 刷新控件
     */
    private RVRefreshView mRefresh;
    /**
     * 加载更多控件
     */
    private RVLoadMoreView mLoadMore;
    ...
    private void init(Context context) {
        ...
        // 初始化刷新、加载控件
        mRefresh = new RVRefreshView(getContext());
        mLoadMore = new RVLoadMoreView(getContext());
        // 设置加载更多点击事件监听
        mLoadMore.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                // 控件为提示状态时，加载更多数据
                if (mLoadMore.getStatus() == RVStatusView.STATUS_TIPS) {
                    onLoadMore();
                }
            }
        });
        ...
        if (!REFRESH_MODE_NONE.equals(mode)) { // 刷新、加载更多功能开启
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE: // 移动
                    if (canRefresh()) { // 刷新功能开启
                        if (pos[0] == 0) { // 滑动到顶部
                            ...
                            if (mRefresh.getStatus() != RVStatusView.STATUS_LOADING) { // 不是正在加载中
                    ...
                    if (canLoadMore()) { // 加载更多功能开启
                        if (mLoadMore.getStatus() == RVStatusView.STATUS_TIPS) { // 数据不足一屏，不做操作
                            return false;
                        }
                        if (pos[1] == innerAdapter.getItemCount() - 1) { // 滑动到底部
                            ...
                            if (mLoadMore.getStatus() != RVStatusView.STATUS_LOADING) { // 不是正在加载中
                ...
                case MotionEvent.ACTION_UP:
                    if (canRefresh()) {
                        if (mRefresh.getStatus() == RVStatusView.STATUS_LOOSEN) {
                            // 松手时如果是释放立即刷新状态，就去调用正在刷新的任务
                            onRefresh();
                        } else if (mRefresh.getStatus() == RVStatusView.STATUS_PULLING) {
                            // 松手时如果是下拉状态，隐藏刷新控件
                            hideRefresh();
                        }
                    }
                    if (canLoadMore()) {
                        if (mLoadMore.getStatus() == RVStatusView.STATUS_LOOSEN) {
                            // 松手时如果是释放立即加载状态，加载更多
                            onLoadMore();
                        } else if (mLoadMore.getStatus() == RVStatusView.STATUS_PULLING) {
                            // 松手时如果是上拉状态，隐藏加载更多控件
                            hideLoadMore();
                        }
                    }
     ...
     */
    private int getPercent(RVStatusView view) {
    ...
    public void onComplete() {
        ...
        if (RVStatusView.STATUS_LOADING == mRefresh.getStatus()) {
            mRefresh.onFinish();
            hideRefresh();
        }

        if (RVStatusView.STATUS_LOADING == mLoadMore.getStatus()) {
            mLoadMore.onFinish();
            hideLoadMore();
        }
    }
    ...
    /**
     * 刷新回调，优先{@link RLRecyclerView.OnRefreshListener}
     */
    ...
    /**
     * 加载更多回调，优先{@link RLRecyclerView.OnLoadMoreListener}
     */
        ...
        public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            switch (viewType) {
                case VIEW_TYPE_REFRESH:
                    holder = new InsideAdapter.SimpleViewHolder(mRefresh);
                    break;
                case VIEW_TYPE_HEADER:
                    holder = new InsideAdapter.SimpleViewHolder(mHeaders.get(headerPosition++));
                    break;
                case VIEW_TYPE_NORMAL:
                    holder = mAdapter.onCreateViewHolder(parent, viewType);
                    break;
                case VIEW_TYPE_FOOTER:
                    holder = new InsideAdapter.SimpleViewHolder(mFooters.get(footerPosition++));
                    break;
                case VIEW_TYPE_LOADMORE:
                    holder = new InsideAdapter.SimpleViewHolder(mLoadMore);
                    break;
                default:
                    holder = new InsideAdapter.SimpleViewHolder(null);
                    break;
            }
    ...
    class HideRefreshTask extends AsyncTask<Void, Integer, Integer> {
        ...
        protected void onProgressUpdate(Integer... values) {
            mRefreshLayoutParams.topMargin = values[0];
            mRefresh.setLayoutParams(mRefreshLayoutParams);
        }

        @Override
        protected void onPostExecute(Integer integer) {
            mRefreshLayoutParams.topMargin = integer;
            mRefresh.setLayoutParams(mRefreshLayoutParams);
            mRefresh.onFinish();
     ...
     * 设置刷新事件监听
     *
     * @param listener 刷新事件监听接口{@link RLRecyclerView.OnRefreshListener}
     */
    public void setOnRVListener(OnRefreshListener listener) {
     ...
     * 设置加载更多事件监听
     *
     * @param listener 加载更多事件监听接口{@link RLRecyclerView.OnLoadMoreListener}
     */
    public void setOnRVListener(OnLoadMoreListener listener) {
     ...
     * 设置刷新、加载更多事件监听
     *
     * @param listener 刷新、加载更多事件监听接口{@link RLRecyclerView.OnRLListener}
     */
    public void setOnRVListener(OnRVListener listener) {
        rlListener = listener;
    }

    private OnRVListener rlListener;
